...
with open(os.path.join("dataset", "fewshot_imgs.json"), "r") as f:
    few_imgs = json.load(f)
    for i in range(len(few_imgs)):
        few_imgs[i] = list(set(few_imgs[i]))
with open(os.path.join("dataset", "img2box.json"), "r") as f:
    img2box = json.load(f)
with open(os.path.join("dataset", "label2class.json"), "r") as f:
...
with torch.no_grad():
    ...
    fns = os.listdir("dataset/VOC2007test/JPEGImages")
    st = 0
    bs = 12
    tt = list()
    while st < len(fns):
    ...
    for nfn in tt:
        ...
        seeds = list()
        for fn in nfn:
            img = Image.open(os.path.join("dataset/VOC2007test/JPEGImages", fn))
            print(fn)
